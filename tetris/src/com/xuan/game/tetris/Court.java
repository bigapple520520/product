 ...
 * 
 * @author xuan
 * @version $Revision: 1.0 $, $Date: 2014-5-30 下午5:13:32 $
 */
public class Court {
    ...
    private final int[][] mCourt = new int[COURT_WIDTH][COURT_HEIGHT];
    ...
    public void paintCourt(Canvas canvas) {
        ...
        paint.setAlpha(0x60);
        canvas.drawBitmap(resourceStore.getCourtBackground(), 0, 0, paint);
        paint.setAlpha(0xee);
        for (int i = 0; i < Court.COURT_WIDTH; i++) {
            for (int j = 0; j < Court.COURT_HEIGHT; j++) {
                if (mCourt[i][j] != 0) {
                    canvas.drawBitmap(resourceStore.getBlock(mCourt[i][j] - 1), BEGIN_DRAW_X + i * Court.BLOCK_WIDTH,
                            BEGIN_DRAW_Y + j * Court.BLOCK_WIDTH, paint);
                }
            }
        }
    }
