...
function mc_admin_url($path, $print = true){
  global $mc_config;
  $url = $mc_config['site_link'].'/admin/'.$path;
  if ($print) {
    echo $url;
    return;
  }
...
function mc_the_summary($print = true) {
  global $mc_data;
  if (!isset($mc_data)) {
    global $mc_post_id;
  ...
  // ½ØÈ¡ÕªÒª
  $index = strrpos($html,SUMMARY_SPLIT);
  $summary = $html;
  if($index != false){
    $summary = substr($html,0,$index);
  }
